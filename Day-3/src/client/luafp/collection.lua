...
function collection.map(func, list)
    ...
    for i, v in ipairs(list) do
        table.insert(built, func(v, i, list))
    end
    return built
end
...
collection.map = func.curry(collection.map, 2)
collection.filter = func.curry(collection.filter, 2)
collection.reduce = func.curry(collection.reduce, 3)
collection.every = func.curry(collection.every, 2)
collection.some = func.curry(collection.some, 2)
